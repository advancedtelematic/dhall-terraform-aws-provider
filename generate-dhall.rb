...
def type_from_resource(resource, parents = [])
  name = resource["type_name"]
  parents = parents.clone.append(name)
  blocks = resource["block_types"].map { |bt| type_from_resource(bt, parents) }
  ...
  Type.new(name, all_fields, blocks, exported_attributes, parents)
end
