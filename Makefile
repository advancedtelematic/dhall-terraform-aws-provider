...
generate-dhall: build
	docker run --rm -it -v $(CURDIR)/default:/usr/src/app/default -v $(CURDIR)/dhall:/usr/src/app/dhall -v $(CURDIR)/schema:/usr/src/app/schema advancedtelematic/dhall-terraform-aws-provider ruby ./generate-dhall.rb
